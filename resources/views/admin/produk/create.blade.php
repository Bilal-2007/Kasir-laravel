...
<script>
    $(document).ready(function() {
        $("#form-create-produk").submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('produk.store') }}", // Gunakan route yang benar
                data: dataForm,
                dataType: "json",
                success: function(data) {
                    const succsessMessage = data.responseJSON?.message || "produk berhasil";
                    Swal.fire({
                        icon: 'success',
                        title: "Success",
                        ...
                        if (result.isConfirmed) {
                            window.location.href = "{{ route('produk.index') }}"; // Redirect setelah sukses
                        }
                    });
                    // Kosongkan input
                ...
                error: function(data) {
                    const errorMessage = data.responseJSON?.message || "Terjadi kesalahan!";
                    Swal.fire({
                        icon: 'error',
                        title: "Error",
                        text: errorMessage,
